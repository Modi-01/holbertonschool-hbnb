...
function initializeLoginPage() {
  ...
  loginForm.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      showMessage(loginForm, 'Invalid credentials', true, 'login-message');
  ...
  detailsSection.innerHTML = `
    ...
    <div class="place-info">
      ...
      <div>
        ...
        <p><strong>Host:</strong> ${place.host_name || 'Unknown Host'}</p>
      </div>
      <div>
        <h3>Amenities</h3>
...
function updatePlacePageReviewAccess(token, placeId) {
  ...
  if (!token) {
    addReviewSection.innerHTML = `
      <h2>Add a Review</h2>
      <a class="details-button" href="index.html">Go to review form</a>
    `;
    return;
  }
